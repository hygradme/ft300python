...
class FT300Stream:
    def __init__(self, port, timeout, zero_reset=True):
        ...
        try:
            client.write_registers(address=410, values=0x200, unit=9)
        finally:
            client.close()

        # Start obtaining stream
        self.ser = serial.Serial(
            port=port, baudrate=19200, bytesize=8, parity="N", stopbits=1)
    ...
    def get_force_torque_raw(self):
        ...
        force_torque = [int.from_bytes(
            raw_bytes[i*2: i*2+2],
            byteorder='little',
            signed=True) / coef_list[i] for i in range(0, 6)
            ]
        return force_torque
